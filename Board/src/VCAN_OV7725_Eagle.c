...
void ov7725_eagle_port_init()
{
    ...
    dma_portx2buff_init(CAMERA_DMA_CH, (void *)&PTD_B0_IN, (void *)ov7725_eagle_img_buff, PTE1, DMA_BYTE1, CAMERA_DMA_NUM, DADDR_KEEPON);

    DMA_DIS(CAMERA_DMA_CH);
    disable_irq(PORTE_IRQn);                        //关闭PTA的中断
	disable_irq(PORTC_IRQn);
    DMA_IRQ_CLEAN(CAMERA_DMA_CH);                   //清除通道传输中断标志位
    DMA_IRQ_EN(CAMERA_DMA_CH);

    port_init(PTE1, ALT1 | DMA_FALLING | PULLDOWN );         //PCLK

    port_init(PTC6, ALT1 | IRQ_RISING  | PULLDOWN | PF);     //场中断，上拉，上降沿触发中断，带滤波
...
void ov7725_eagle_get_img()
{
    ...
    PORTE_ISFR = ~0;                        //写1清中断标志位(必须的，不然回导致一开中断就马上触发中断)
	PORTC_ISFR = ~0;
    enable_irq(PORTE_IRQn);                         //允许PTA的中断
